...
const description = (flags: flags) => {
  ...
  const SHORTEST_PATH = 42; // Took me at least 42 moves to get the "good ending"

  const pathMsg = GM.stepCounter < SHORTEST_PATH
    ? "Impressive. You've managed to beat the developer's shortest path. Please tell me how you did it."
    : GM.stepCounter === SHORTEST_PATH
      ? "Well done. You didn't bother looking at stuff or immersing yourself in the environment. You just blitzed through the entire game with fearsome efficiency, matching my own final score."
      : "If you're up to the challenge, try finishing the game in 42 actions or less."
